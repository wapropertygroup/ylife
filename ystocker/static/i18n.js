...
const I18n = (() => {
  ...
  const LANGS = {
    ...
    'dca.peers_title':     { en: 'Peers, scored the same way', zh: '同组公司的同口径评分' },
    'dca.peers_note':      { en: 'Every V here is a company against its own history, so a higher number means "cheaper than this company usually is", not "cheaper than the row above". A ✻ marks a peer scored on a different template, whose V is therefore built from a different set of multiples. The P/E column is the one figure that does compare across the group — it is the same multiple the peer factor is ranked on.',
                             zh: '这里的每个 V 都是公司与自身历史相比，数值更高只代表「比它自己平时更便宜」，并不代表「比上一行更便宜」。带 ✻ 的公司使用了不同的估值模板，其 V 来自另一组倍数指标。只有市盈率一列可以横向比较——它正是 peer 因子所用的同一个倍数。' },
    'dca.peers_loading':   { en: 'Scoring peers…', zh: '正在为同组公司评分……' },
    'dca.peers_failed':    { en: 'Could not load peers.', zh: '无法加载同组公司。' },
    'dca.peers_no_group':  { en: 'This ticker is in no peer group, so there is nothing to compare it against.',
                             zh: '该股票不属于任何同业分组，因此没有可对比的对象。' },
    'dca.peers_none':      { en: 'No peer in {g} has been scored yet. Open one below to score it.',
                             zh: '{g} 分组中暂无已评分的公司。点击下方任一代码即可为其评分。' },
    'dca.peers_col_name':  { en: 'Company', zh: '公司' },
    'dca.peers_col_model': { en: 'Model', zh: '估值模型' },
    'dca.peers_col_v':     { en: 'V', zh: 'V' },
    'dca.peers_col_band':  { en: 'Band', zh: '估值区间' },
    'dca.peers_col_pe':    { en: 'P/E', zh: '市盈率' },
    'dca.peers_col_fwd_pe':{ en: 'Fwd P/E', zh: '预测市盈率' },
    'dca.peers_col_ttm_pe':{ en: 'P/E (TTM)', zh: '滚动市盈率' },
    'dca.peers_col_mult':  { en: 'Multiplier', zh: '调节系数' },
    'dca.peers_col_amount':{ en: 'Contribution', zh: '本次定投' },
    'dca.peers_other_model': { en: 'Scored on a different template, so its V is built from a different set of multiples.',
                             zh: '使用了不同的估值模板，其 V 分数来自另一组倍数指标。' },
    'dca.peers_truncated': { en: '{n} further scored peers in this group are not shown — they are the dearest against their own history, since this table is sorted cheapest first.',
                             zh: '该分组还有 {n} 家已评分公司未显示——本表按「相对自身历史最便宜」排序，未显示的是最贵的几家。' },
    'dca.peers_unscored_title': { en: 'Not scored yet', zh: '尚未评分' },
    'dca.peers_unscored_basis': { en: ' · {pe} shown', zh: ' · 显示{pe}' },
    'dca.peers_unscored_note': { en: 'Nothing is built here on your behalf — each name is six filings reads, and building them off a side panel would push them into the tracked universe and evict something else. Open one to score it.',
                             zh: '此处不会自动为你构建：每只股票需要读取六份财报数据，若在侧栏批量构建会把它们挤进受跟踪名单并顶掉其他股票。点击代码即可单独评分。' },
    'dca.peers_r_not_built': { en: 'No reconstruction on disk yet — open it to build one.',
                             zh: '尚未重建过估值历史——点击即可构建。' },
    'dca.peers_r_unavailable': { en: 'Publishes no statements this engine can rank (most ETFs).',
                             zh: '没有本引擎可用于排名的财务报表（多数 ETF 属于此类）。' },
    ...
    // The basis of the headline V, stated by weight rather than factor count.
    'dca.sb_label':      { en: 'Ranked on',            zh: '排名口径' },
    'dca.sb_ttm':        { en: 'trailing (TTM) history', zh: '滚动（TTM）历史' },
    'dca.sb_fwd':        { en: 'forward history',      zh: '预测口径历史' },
    ...
    'dca.pin_on_tip':    { en: 'Kept in the ranked list and never dropped to make room. {n} of {max} slots kept. Click to release it back to ordinary recency.',
                           zh: '已固定在榜单中，不会为腾位置而被删除。已保留 {n}/{max} 个名额。点击可恢复为按最近访问淘汰。' },
    'dca.pin_off_tip':   { en: 'The list is capped, and the least-recently-opened name is dropped when a new one is added. Keep this one so it is dropped last. {n} of {max} slots kept.',
                           zh: '榜单有数量上限，新增标的时会淘汰最久未打开的一个。保留后它将最后才被淘汰。已保留 {n}/{max} 个名额。' },
    'dca.pin_err_no_room': { en: 'Every keepable slot is taken ({max}). Release another name first — the cap is a daily data budget, not a storage limit.',
                             zh: '可保留的名额已用完（共 {max} 个）。请先释放一个——这个上限是每日数据用量预算，不是存储限制。' },
    'dca.pin_err_not_tracked': { en: 'This name is not in the ranked list yet. It joins once a rebuild produces a score for it.',
                                 zh: '该标的尚未进入榜单。重建并成功打分后会自动加入。' },
    'dca.pin_err_seed':  { en: 'This name is part of the standard set and is never dropped.',
                           zh: '该标的属于标准组合，本来就不会被淘汰。' },
    'dca.pin_err_forbidden': { en: 'Sign in with an account permitted to change the shared list.',
                               zh: '请使用有权修改共享榜单的账号登录。' },
    'dca.pin_err_unknown': { en: 'Could not save.', zh: '保存失败。' },
    ...
    'dcx.search_note':  { en: 'Any listed company works. A name that scores is added to the table below and refreshed daily from then on. Funds are not suggested — an ETF publishes no statements, so there is nothing to reconstruct a valuation history from. First build takes about a minute.',
                          zh: '任何上市公司都可以。评分成功的股票会加入下方榜单，并从此每日刷新。基金不会出现在推荐中——ETF 不披露财报，无法重建估值历史。首次构建约需一分钟。' },
    'dcx.recent':       { en: 'Recent', zh: '最近查看' },
    'dcx.not_scorable': { en: 'A fund publishes no statements, so it has no valuation history to score.',
                          zh: '基金不披露财报，没有可用于评分的估值历史。' },
    ...
    // GICS panel (/api/gics). Built from the S&P 500's own constituents, so it
    // can reach the 25 industry groups no sector ETF isolates.
    'markets.gics_title':      { en: 'GICS Sectors & Industry Groups — S&P 500', zh: 'GICS 行业表现 — 标普500一级行业与行业组' },
    'markets.gics_desc':       { en: 'The rotation grid above tracks eleven sector ETFs. This splits each GICS sector into its industry groups, where rotation often actually happens — semiconductors against software, banks against insurers — computed from the S&P 500’s own constituents, cap-weighted, total return.',
                                 zh: '上方的轮动表跟踪 11 只板块 ETF。这里把每个 GICS 一级行业拆到行业组——轮动往往发生在这一层：半导体对软件、银行对保险——由标普500成分股按市值加权计算，含股息。' },
    'markets.gics_mode_ret':   { en: 'Return',        zh: '涨跌幅' },
    'markets.gics_mode_rel':   { en: 'vs S&P 500',    zh: '相对标普500' },
    'markets.gics_mode_wchg':  { en: 'Weight change', zh: '权重变化' },
    'markets.gics_col_name':   { en: 'Sector / industry group', zh: '行业 / 行业组' },
    'markets.gics_col_weight': { en: 'Weight', zh: '权重' },
    'markets.gics_col_n':      { en: 'Names',  zh: '成分股' },
    'markets.gics_index':      { en: 'S&P 500', zh: '标普500' },
    'markets.gics_p_1D':  { en: '1D',  zh: '1日' },
    'markets.gics_p_1W':  { en: '1W',  zh: '1周' },
    'markets.gics_p_1M':  { en: '1M',  zh: '1月' },
    'markets.gics_p_3M':  { en: '3M',  zh: '3月' },
    'markets.gics_p_6M':  { en: '6M',  zh: '6月' },
    'markets.gics_p_YTD': { en: 'YTD', zh: '年初至今' },
    'markets.gics_p_1Y':  { en: '1Y',  zh: '1年' },
    'markets.gics_sort_tip':   { en: 'Sort by this column', zh: '按此列排序' },
    'markets.gics_toggle_tip': { en: 'Show or hide its industry groups', zh: '展开或收起行业组' },
    'markets.gics_asof':       { en: 'As of the {date} close.', zh: '截至 {date} 收盘。' },
    'markets.gics_stale':      { en: 'Not refreshed since — a rebuild is pending.', zh: '此后尚未刷新，等待重新计算。' },
    'markets.gics_note_etf':   { en: 'Can differ from the sector ETFs above: the Select Sector SPDRs cap single names, and three companies are over half of S&P 500 IT. These are the uncapped sectors themselves.',
                                 zh: '可能与上方的板块 ETF 不同：SPDR 板块 ETF 对单一个股设有权重上限，而三家公司就占了标普500信息技术的一半以上。这里是未设上限的行业本身。' },
    'markets.gics_note_method': { en: 'Weights from SPY’s holdings ({date}), drifted with prices since. Each horizon counts only companies already in the index at its start, so a name added this year does not lend its sector the gains it made before joining.',
                                  zh: '权重取自 SPY 持仓（{date}），此后随价格漂移。每个区间只计入期初已在指数内的公司——今年才纳入的成分股，不会把入选前的涨幅算进所属行业。' },
    'markets.gics_note_wchg':  { en: 'Weight change is the shift in share of the index from relative performance alone, in percentage points; additions and deletions are left out.',
                                 zh: '权重变化只反映相对表现带来的指数占比变化，单位为百分点，不含成分股调入调出。' },
    'markets.gics_note_small': { en: 'A group with few names can differ from S&P’s official index by more than a point, because one disputed classification is a large share of it.',
                                 zh: '成分股较少的行业组，与标普官方指数可能相差超过 1 个百分点：一家公司的分类差异就占很大比重。' },
    'markets.gics_loading':    { en: 'Loading…', zh: '加载中…' },
    'markets.gics_warming':    { en: 'Computing from the constituents’ prices — this takes a minute or two after a restart.',
                                 zh: '正在根据成分股价格计算——重启后需要一两分钟。' },
    'markets.gics_gave_up':    { en: 'Still computing. Reload in a few minutes.', zh: '仍在计算中，请几分钟后刷新。' },
    'markets.gics_unavailable': { en: 'The GICS breakdown is unavailable right now.', zh: 'GICS 行业数据暂不可用。' },
    // Official GICS names (structure effective March 2023). Keyed by code, not
    // by name: a name has '&', commas and parentheses, a code has none.
    'gics.10': { en: 'Energy',                 zh: '能源' },
    'gics.15': { en: 'Materials',              zh: '原材料' },
    'gics.20': { en: 'Industrials',            zh: '工业' },
    'gics.25': { en: 'Consumer Discretionary', zh: '非必需消费品' },
    'gics.30': { en: 'Consumer Staples',       zh: '日常消费品' },
    'gics.35': { en: 'Health Care',            zh: '医疗保健' },
    'gics.40': { en: 'Financials',             zh: '金融' },
    'gics.45': { en: 'Information Technology', zh: '信息技术' },
    'gics.50': { en: 'Communication Services', zh: '通信服务' },
    'gics.55': { en: 'Utilities',              zh: '公用事业' },
    'gics.60': { en: 'Real Estate',            zh: '房地产' },
    'gics.1010': { en: 'Energy',                                        zh: '能源' },
    'gics.1510': { en: 'Materials',                                     zh: '原材料' },
    'gics.2010': { en: 'Capital Goods',                                 zh: '资本品' },
    'gics.2020': { en: 'Commercial & Professional Services',            zh: '商业和专业服务' },
    'gics.2030': { en: 'Transportation',                                zh: '运输' },
    'gics.2510': { en: 'Automobiles & Components',                      zh: '汽车与汽车零部件' },
    'gics.2520': { en: 'Consumer Durables & Apparel',                   zh: '耐用消费品与服装' },
    'gics.2530': { en: 'Consumer Services',                             zh: '消费者服务' },
    'gics.2550': { en: 'Consumer Discretionary Distribution & Retail',  zh: '非必需消费品分销与零售' },
    'gics.3010': { en: 'Consumer Staples Distribution & Retail',        zh: '日常消费品分销与零售' },
    'gics.3020': { en: 'Food, Beverage & Tobacco',                      zh: '食品、饮料与烟草' },
    'gics.3030': { en: 'Household & Personal Products',                 zh: '家庭与个人用品' },
    'gics.3510': { en: 'Health Care Equipment & Services',              zh: '医疗保健设备与服务' },
    'gics.3520': { en: 'Pharmaceuticals, Biotechnology & Life Sciences', zh: '制药、生物科技与生命科学' },
    'gics.4010': { en: 'Banks',                                         zh: '银行' },
    'gics.4020': { en: 'Financial Services',                            zh: '金融服务' },
    'gics.4030': { en: 'Insurance',                                     zh: '保险' },
    'gics.4510': { en: 'Software & Services',                           zh: '软件与服务' },
    'gics.4520': { en: 'Technology Hardware & Equipment',               zh: '技术硬件与设备' },
    'gics.4530': { en: 'Semiconductors & Semiconductor Equipment',      zh: '半导体与半导体设备' },
    'gics.5010': { en: 'Telecommunication Services',                    zh: '电信服务' },
    'gics.5020': { en: 'Media & Entertainment',                         zh: '媒体与娱乐' },
    'gics.5510': { en: 'Utilities',                                     zh: '公用事业' },
    'gics.6010': { en: 'Equity REITs',                                  zh: '股权房地产投资信托' },
    'gics.6020': { en: 'Real Estate Management & Development',          zh: '房地产管理与开发' },
    ...
    'markets.cta_us_equities': { en: 'US equities', zh: '美国股票' },
    'markets.cta_us_reading': { en: 'US {value} · {date}', zh: '美股 {value} · {date}' },
    ...
    'markets.cta_position_chart': { en: 'Reported CTA equity net length ($bn)', zh: '已披露 CTA 股票净多仓（十亿美元）' },
    ...
    // ── /posts ─────────────────────────────────────────────────────────
    'nav.inbox':            { en: 'Posts', zh: '推送' },
    'inbox.title':          { en: 'Posts', zh: '推送' },
    'inbox.subtitle':       { en: 'Posted here by external systems. Markdown and HTML are rendered.',
                              zh: '外部系统推送到这里的内容。支持 Markdown 与 HTML 渲染。' },
    'inbox.signin_title':   { en: 'Sign in to read the posts', zh: '登录后查看推送' },
    'inbox.signin_body':    { en: 'Anything posted here is private to this site. Writing is authenticated separately, with a token.',
                              zh: '推送到这里的内容不对外公开。写入走另一套鉴权，使用 token。' },
    'inbox.signin_btn':     { en: 'Sign In', zh: '登录' },
    'inbox.loading':        { en: 'Loading…', zh: '加载中…' },
    'inbox.refresh':        { en: '↻ Refresh', zh: '↻ 刷新' },
    'inbox.all':            { en: 'All', zh: '全部' },
    'inbox.n_messages':     { en: '{n} posts', zh: '{n} 条推送' },
    'inbox.empty':          { en: 'Nothing here yet.', zh: '还没有推送。' },
    'inbox.empty_hint':     { en: 'Post something to get started — the example below works as-is once the token is set.',
                              zh: '推送一条即可开始——设好 token 后，下面的示例可以直接用。' },
    'inbox.open_link':      { en: 'Open link', zh: '打开链接' },
    'inbox.view_list':      { en: 'List',  zh: '列表' },
    'inbox.view_board':     { en: 'Board', zh: '看板' },
    'inbox.no_source':      { en: 'no source', zh: '未标来源' },
    'inbox.more':           { en: 'Show more', zh: '展开' },
    'inbox.less':           { en: 'Show less', zh: '收起' },
    'inbox.src_refused':    { en: '{kind} address not supported ({scheme}:)',
                              zh: '{kind}地址不被支持（{scheme}:）' },
    'inbox.src_refused_tip':{ en: 'Only http(s) and data:image addresses are loaded. A cid: or file: address refers to something that is not on this server, so it can never load — the sender has to supply a public URL.',
                              zh: '只加载 http(s) 和 data:image 地址。cid: 或 file: 指向的东西不在这台服务器上，永远加载不出来——需要发送方改成公开可取的 URL。' },
    'inbox.media_img':      { en: 'Image', zh: '图片' },
    'inbox.media_video':    { en: 'Video', zh: '视频' },
    'inbox.media_audio':    { en: 'Audio', zh: '音频' },
    'inbox.media_source':   { en: 'Media', zh: '媒体' },
    ...
    'inbox.level_info':     { en: 'info',    zh: '信息' },
    'inbox.level_success':  { en: 'success', zh: '成功' },
    'inbox.level_warn':     { en: 'warning', zh: '警告' },
    'inbox.level_error':    { en: 'error',   zh: '错误' },
    'inbox.err_signed_out': { en: 'Sign in to read the posts.', zh: '请先登录再查看推送。' },
    'inbox.err_store_unavailable': { en: 'The store is unreachable, so this list is not empty — it is unknown. Nothing has been lost.',
                              zh: '存储暂时无法访问，所以这里不是「没有内容」而是「暂时读不到」。数据未丢失。' },
    'inbox.err_unknown':    { en: 'Could not load the posts.', zh: '无法加载推送。' },
