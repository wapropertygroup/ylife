...
const I18n = (() => {
  ...
  const LANGS = {
    ...
    'adca.title':      { en: "This period's contribution, per holding", zh: '本期各持仓建议投入' },
    'adca.intro':      { en: 'Base × M_valuation × M_earnings × M_portfolio. The concentration term uses your 穿透 weight, not the line weight — holding NVDA directly and an S&P fund besides is more NVDA than the row says, and this is the one place that shows both terms pulling against each other on your own money.',
                         zh: '基础金额 × M_估值 × M_盈利 × M_组合。集中度一项使用你的穿透权重，而非单行权重 — 直接持有 NVDA 又持有标普基金，实际 NVDA 敞口高于该行显示值。这是唯一能看到估值与集中度两项在自己资金上相互制衡的地方。' },
    ...
    'adca.none':       { en: 'Add a holding and its contribution will be sized here.',
                         zh: '添加持仓后，这里会给出对应的投入金额。' },
    'adca.col_symbol': { en: 'Symbol', zh: '代码' },
    'adca.col_weight': { en: 'Weight (穿透)', zh: '权重（穿透）' },
    ...
    'adca.col_amount': { en: 'Contribution', zh: '投入金额' },
    'adca.st_fund':    { en: 'Fund — not scored', zh: '基金 — 不评分' },
    ...
    'adca.footnote':   { en: 'Valuation sets the pace of buying; it never decides whether to buy. Funds are not scored — the engine ranks a company against its own multiple history, and an ETF has no P/E of its own here. A name with no published statements, or too few measurable factors, shows no multiplier rather than a guessed one.',
                         zh: '估值调节的是投入速度，不决定买或不买。基金不评分 — 引擎将公司与其自身倍数历史比较，而 ETF 在此框架下没有自己的市盈率。没有公开财报或可测量因子不足的股票会留空，而不是给出一个猜测值。' },
