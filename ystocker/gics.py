...
:func:`write_snapshot`) — the same "static, refreshed on a code change" choice
breadth.py makes for its universe, and for the same reason: a constituent-list
scraper does not belong on a path that runs every day unattended.
...
def write_snapshot(out: Path = SNAPSHOT_FILE) -> Path:
    ...
    a few times a year, and after a large reconstitution:
        ...
        python -m ystocker.gics --write-snapshot

    Needs outbound access to en.wikipedia.org and www.ssga.com, which the box
    has and a sandboxed laptop may not.
    """
...
def performance(closes: Any, snap: dict[str, Any], *, now: Optional[float] = None) -> dict[str, Any]:
    ...
    def on_or_before(ts: Any) -> Optional[Any]:
        pos = days.searchsorted(ts, side="right") - 1
        return days[pos] if pos >= 0 else None

    prior = days[days < end]
    bases: dict[str, Any] = {
        "1D": prior[-1] if len(prior) else None,
        "1W": on_or_before(end - pd.Timedelta(days=7)),
        "1M": on_or_before(end - pd.DateOffset(months=1)),
        "3M": on_or_before(end - pd.DateOffset(months=3)),
        "6M": on_or_before(end - pd.DateOffset(months=6)),
        # The last close of the previous year, not the first close of this one:
        # starting from 2 January's close drops that session's move from YTD.
        "YTD": on_or_before(pd.Timestamp(end.year - 1, 12, 31)),
        "1Y": on_or_before(end - pd.DateOffset(years=1)),
    }
